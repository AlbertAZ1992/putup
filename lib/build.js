...
async function build(options) {
  ...
  if (postsList && postsList.length) {
    ...
    console.log(`\nRendering all posts...`);
    await renderAllPosts(postsList, layoutDir, tmpDir);
...
async function renderAllPosts(postsList, layoutDir, targetDir) {
  for (postObj of postsList) {
    let layout = path.resolve(layoutDir, postObj.layout,  'index.html');
    let js = path.resolve(layoutDir, postObj.layout, 'index.js');
    let html = renderPostToHtml(layout, { autoescape: false, useCache: true }, postObj);

...
async function build(options) {
  ...
  let { postsList, aboutObj } = await resolvePosts(postDir);
  ...
  if (postsList && postsList.length) {
    ...
    await renderAboutPage(aboutObj, layoutDir, tmpDir, siteInfo)
...
async function renderAboutPage(aboutObj, layoutDir, targetDir, siteInfo) {
  let layout = path.resolve(layoutDir, 'about', 'index.html');
  let entry = path.resolve(layoutDir, 'about', 'index.js');
  let theme = path.resolve(layoutDir, 'theme');
  let html = renderPostToHtml(layout, { autoescape: false }, {...aboutObj, site: siteInfo});
  await fs.outputFile(path.resolve(targetDir, 'about/index.html'), html);
  await fs.copy(entry, path.resolve(targetDir, 'about/index.js'));
  await fs.copy(theme, path.resolve(targetDir, 'about'));
}
module.exports = (...args) => {
